...
| `config.py` | **설정 읽기.** 어떤 공개 근거 파일을 쓸지, Google AI를 켤지, 어떤 모델을 고를 수 있는지, 몇 초 기다릴지 | 설정 항목을 추가하거나 AI 응답 대기 시간 기본값을 바꿀 때 |
...
| `llm/` | **AI 참고 의견**: Google AI Studio API에 묻고, 답을 검사하고, 모델 이름표를 관리 | [README](llm/README.md) |
...
| `app.py` | FastAPI 생성, 정적 파일 마운트, 라우터 등록, 요청 뒤 세션 상태 저장 middleware |
...
| `config.py` | `load_settings()`. 아래 표 참고 |
...
- Vercel 세션 설정은 `web/session.py`가 읽는다. Vercel에서는 Redis 설정이 없으면 작업 화면을 503으로 막고, 로컬은 메모리를 사용한다.
