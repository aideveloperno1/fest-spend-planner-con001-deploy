...
AI 참고 의견을 쓰려면 Ollama를 설치하고 배포본의 기본 모델을 최초 한 번 받습니다.

```powershell
ollama pull exaone3.5:7.8b
```

- `run_deploy.ps1`이 로컬 주소와 모델 이름을 자동으로 설정합니다.
- 다른 모델을 쓸 때만 `.env.example`을 참고해 환경변수를 직접 설정합니다.
- AI 응답을 기다리는 최대 시간은 기본 120초입니다(`PSM_LLM_TIMEOUT_S`로 바꿈).

## 3. 실행

```powershell
.\run_deploy.ps1             # 공개 합성 자료 + 로컬 LLM
.\run_deploy.ps1 -NoLlm      # 공개 합성 자료, AI 참고 의견 없음
```

브라우저에서 `http://127.0.0.1:8000`을 엽니다. 화면 위쪽에 **"시연용 합성 수치 · demo-2.1-003"** 표시가 보이면 공개 전달본으로 동작 중입니다.
